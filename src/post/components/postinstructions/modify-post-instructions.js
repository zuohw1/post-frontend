...
import {
  Form, Button, Input, Radio, Checkbox, Select, Modal, DatePicker, Icon,
} from 'antd';
...
export default (props) => {
  ...
  const add = () => {
    const oCruxDuty = document.getElementsByClassName('crux-duty-top');
    for (let i = 0; i < oCruxDuty[0].classList.length; i += 1) {
      ...
      >
        {getFieldDecorator(`names[${k}]`, {
          ...
          <span>
            ...
            <Input
              placeholder=""
              style={{ width: '66.6%', marginRight: 8, position: 'relative' }}
              suffix={<Icon type="check-square" style={{ color: 'rgba(0,0,0,.25)' }} onClick={onInstructionsView} />}
            />
          </span>,
        )}
        {keys.length > 1 ? (
